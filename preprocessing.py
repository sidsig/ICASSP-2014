...
class PreProcessor():
	def __init__(self,dataset_dir,):
		...
		self.ground_truth = pickle.load(open(os.path.join(self.list_dir,'ground_truth'),'r'))
		self.load_data()
	...
	def load_data(self,):
		...
		self.initial_shape = self.features.shape[1:]
		self.n_per_example = numpy.prod(self.features.shape[1:-1])
		self.n_features = self.features.shape[-1]
		features,filenames = self.flatten_data(features,filenames)
	...
	def prepare_subdataset(self,split='train',unflatten=False,randomize=False):
		...
		targets = self.make_targets(filenames)
		if unflatten:
